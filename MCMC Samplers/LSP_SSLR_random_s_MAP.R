...
# wrapper function -------------------------------------------------------
lsp_random_ssl_map <- function(X, y, weights, a_s = 1, b_s = NA,
                               E_space = c(1, 2, 3), c_space = c(0, 1),
                               lambda1 = NULL, lambda0s = NULL,
                               variance = "unknown", max_iter = 500,
                               eps = 0.001) {
  n <- nrow(X)
  p <- ncol(X)
...
construct_v_vec <- function(weights, c_val, eta) {
  if (c_val == 0) return(rep(1.0, length(weights)))
  w_eta <- weights^eta
...
threshold_func_vec <- function(theta_vec, sigma2, lambda1, lambda0, xnorm_vec) {
  p <- length(theta_vec)
  if (lambda1 == lambda0) return(rep(sigma2 * lambda1, p))
...
SSL_thresholding <- function(z, beta_val, lambda0, lambda1, theta_j, norm, delta, sigma2) {
  if (abs(z) <= delta) return(0)
  if (lambda1 == lambda0) {
    lambda <- lambda1
  } else {
    ratio <- ((1 - theta_j) / theta_j) * (lambda0 / lambda1) *
...
compute_log_posterior <- function(y, X, beta_final, sigma_final,
                                  v_vec, lambda1, lambda0_final, a_s, b_s) {
  n <- length(y)
  p <- length(beta_final)
...
lsp_ssl_random_descent <- function(X, y, initialbeta, variance = "unknown",
                                 lambda1, lambda0s, v_vec, a_s, b_s,
                                 sigma, min_sigma2, eps, max_iter, count_max) {
  n <- nrow(X)
  p <- ncol(X)
  L <- length(lambda0s)
  ...
  for (l in 1:L) {
    ...
    if (l > 1L && variance == "unknown") {
      if (iter_vec[l - 1L] < 100L) {
        ...
        if (sigma2 < min_sigma2) { sigma2 <- sigma2_init; estimate_sigma <- FALSE }
      } else {
        estimate_sigma <- FALSE
    ...
    while (iter_vec[l] < max_iter) {
      ...
      while (iter_vec[l] < max_iter) {
        ...
        for (j in 1:p) {
          ...
          if (e1[j]) {
            ...
            b_val       <- SSL_thresholding(z[j], a_vec[j], lambda0, lambda1,
                                            theta_vec[j], xnorm[j], delta[j], sigma2)
            b_mat[j, l] <- b_val
      ...
      if (violations_rest > 0L) next

      # ── Finalise ──────────────────────────────────────────────────────
      if (p < thres) r <- y - as.vector(X %*% a_vec)
      b_mat[, l] <- a_vec
      loss[l]    <- sum(r^2)
  ...
  list(beta = b_mat, loss = loss, iter = iter_vec, sigmas = sigmas, s_path = s_path)
}

# ── Grid search wrapper ───────────────────────────────────────────────────────

find_MAP_hyperparams <- function(X, y, weights, a_s, b_s,
                                 E_space, c_space, lambda1, lambda0s, initialbeta, variance, sigma,
                                min_sigma2, eps, max_iter, count_max) {
  L <- length(lambda0s)
  ...
  for (c_val in c_space) {
    for (eta_val in E_space) {
      ...
      res   <- lsp_ssl_random_descent(X, y, v_vec = v_vec, a_s = a_s, b_s = b_s, variance  = variance,
                                    lambda1 = lambda1, lambda0s = lambda0s, initialbeta = initialbeta, sigma = sigma,
                                min_sigma2 = min_sigma2, eps = eps, max_iter = max_iter, count_max = count_max)
      all_paths[[combo_idx]] <- list(c_val = c_val, eta_val = eta_val,
                                     v_vec = v_vec, res = res)
  ...
  for (l in 1:L) {
    best_score   <- -Inf
    best_params_l <- list(lambda0 = lambda0s[l], c = NA, eta = NA,
                          s = NA, beta = NULL, sigma = NA, score = NA)

    for (path in all_paths) {
      beta_l  <- path$res$beta[, l]
      sigma_l <- path$res$sigmas[l]

      if (is.na(sigma_l) || sigma_l <= 0 || anyNA(beta_l)) next

      current_score <- compute_log_posterior(
        y = y, X = X, beta_final = beta_l, sigma_final = sigma_l,
        v_vec = path$v_vec, lambda1 = lambda1, lambda0_final = lambda0s[l],
        a_s = a_s, b_s = b_s
      )
