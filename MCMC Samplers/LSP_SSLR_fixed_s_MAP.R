...
# wrapper function -------------------------------------------------------
lsp_fixed_ssl_map <- function(X, y, weights, s_fixed = 0.5,
...
compute_log_posterior_fixed <- function(y, X, beta_final, sigma_final,
                                        v_vec, lambda1, lambda0_final, s_fixed) {
  n <- length(y)
  p <- length(beta_final)
...
lsp_ssl_fixed_descent <- function(X, y, initialbeta, variance = "unknown",
                                 lambda1, lambda0s, v_vec, s_fixed,
...
find_MAP_fixed_hyperparams <- function(X, y, weights, s_fixed,
  ...
  for (c_val in c_space) {
    for (eta_val in E_space) {
      ...
      res   <- lsp_ssl_fixed_descent(X, y, v_vec = v_vec, s_fixed = s_fixed, variance = variance,
  ...
  for (l in 1:L) {
    ...
    best_params_l <- list(lambda0 = lambda0s[l], c = NA, eta = NA,
                          s = s_fixed, beta = NULL, sigma = NA, score = NA)
